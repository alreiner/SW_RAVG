...
CBI.simple<-gamlss(formula=CBI.B~S_EA_rdnbr_no, nu.formula=~S_EA_rdnbr_no, tau.formula=~S_EA_rdnbr_no,family=BEINF,data=na.omit(SWRAVG_field_train))
...
df.p0.CBI.s<-data.frame(p0.CBI.s,SWRAVG_field_test$S_EA_rdnbr_no)
Limit_0<-df.p0.CBI.s$SWRAVG_field_test.S_EA_rdnbr_no[nearest(df.p0.CBI.s$p0.CBI.s, 0.5, outside = TRUE)] 
print(Limit_0) #see the value

yest.CBI.s<-ifelse(SWRAVG_field_test$S_EA_rdnbr_no<Limit_0,0,yest.CBI.s.gamlss)
...
SWRAVG_field_test$CBI.cat=cut(SWRAVG_field_test$CBI.B,
                breaks=c(-Inf,0.0333,0.4167,0.75,Inf),
                labels=c("0-0.1","0.1-1.25","1.25-2.25","2.25-3"))

#then categorize the predicted data  
yest.CBI.s.cat=cut(yest.CBI.s,
                breaks=c(-Inf,0.03,0.4,0.75,Inf),
                labels=c("0-0.1","0.1-1.25","1.25-2.25","2.25-3"))


CBI.testMSE.S.0<-mean((SWRAVG_field_test$CBI.B-yest.CBI.s)^2) 
...
plot(yest.CBI.s,SWRAVG_field_test$CBI.B, xlab="yest for simple gamlss on model set", ylab="actual", main = "predicted vs actual values of CBI for simple gamlss with limits")
...
BA.simple<-gamlss(formula=pdBA~S_EA_rdnbr_no, sigma.formula=~S_EA_rdnbr_no,nu.formula=~S_EA_rdnbr_no, tau.formula=~S_EA_rdnbr_no,family=BEINF,data=na.omit(SWRAVG_field_train))
...
#create a dataframe to look for rdnbr value closest to 50% probability of zero to use in assigning zeros.
df.p0.BA.s<-data.frame(p0.BA.s,SWRAVG_field_test$S_EA_rdnbr_no)
Limit_0<-df.p0.BA.s$SWRAVG_field_test.S_EA_rdnbr_no[nearest(df.p0.BA.s$p0.BA.s, 0.5, outside = TRUE)] 
print(Limit_0) #see the value

#for BA and CC also look for cutpoint above which probability of 1 is greater than 50%
df.p1.BA.s<-data.frame(p1.BA.s,SWRAVG_field_test$S_EA_rdnbr_no)
Limit_1<-df.p1.BA.s$SWRAVG_field_test.S_EA_rdnbr_no[nearest(df.p1.BA.s$p1.BA.s, 0.5, outside = TRUE)] 
print(Limit_1)
 
 #create predictions that include true zero's and 1's along with continuous response
yest.BA.s<-ifelse(SWRAVG_field_test$S_EA_rdnbr_no<Limit_0,0,ifelse(SWRAVG_field_test$S_EA_rdnbr_no>Limit_1,1,yest.BA.s.gamlss)) 

BA.testMSE.S.0<-mean((SWRAVG_field_test$pdBA-yest.BA.s)^2) 
...
SWRAVG_field_test$pdBA.cat=cut(SWRAVG_field_test$pdBA,
                breaks=c(-Inf,0,0.1,0.25,0.5,0.75,1,Inf),
                labels=c("0","0-10%","10-25%","25-50%","50-75%", "75-100%","100%"))

#then categorize the predicted data  
yest.BA.s.cat=cut(yest.BA.s,
                breaks=c(-Inf,0,0.1,0.25,0.5,0.75,1,Inf),
                labels=c("0","0-10%","10-25%","25-50%","50-75%", "75-100%","100%"))
...
adj.lim.pdFVSVU.simple<-gamlss(formula=adj.lim.pdFVSVU~S_EA_rdnbr_no, sigma.formula=~S_EA_rdnbr_no,nu.formula=~S_EA_rdnbr_no, tau.formula=~S_EA_rdnbr_no,family=BEINF,data=na.omit(SWRAVG_field_train))
...
#create a dataframe to look for rdnbr value closest to 50% probability of zero to use in assigning zeros.
df.p0.adj.lim.pdFVSVU.s<-data.frame(p0.adj.lim.pdFVSVU.s,SWRAVG_field_test$S_EA_rdnbr_no)
Limit_0<-df.p0.adj.lim.pdFVSVU.s$SWRAVG_field_test.S_EA_rdnbr_no[nearest(df.p0.adj.lim.pdFVSVU.s$p0.adj.lim.pdFVSVU.s, 0.5, outside = TRUE)] 
print(Limit_0) #see the value

#for PDFVSVU and CC also look for cutpoint above which probability of 1 is greater than 50%
df.p1.adj.lim.pdFVSVU.s<-data.frame(p1.adj.lim.pdFVSVU.s,SWRAVG_field_test$S_EA_rdnbr_no)
Limit_1<-df.p1.adj.lim.pdFVSVU.s$SWRAVG_field_test.S_EA_rdnbr_no[nearest(df.p1.adj.lim.pdFVSVU.s$p1.adj.lim.pdFVSVU.s, 0.5, outside = TRUE)] 
print(Limit_1)
 
 #create predictions that include true zero's and 1's along with continuous response
yest.adj.lim.pdFVSVU.s<-ifelse(SWRAVG_field_test$S_EA_rdnbr_no<Limit_0,0,ifelse(SWRAVG_field_test$S_EA_rdnbr_no>Limit_1,1,yest.adj.lim.pdFVSVU.s.gamlss)) 

adj.lim.pdFVSVU.testMSE.S.0<-mean((SWRAVG_field_test$adj.lim.pdFVSVU-yest.adj.lim.pdFVSVU.s)^2) 
...
#then categorize the predicted data  
yest.adj.lim.pdFVSVU.s.cat=cut(yest.adj.lim.pdFVSVU.s,
                breaks=c(-Inf,0,0.1,0.25,0.5,0.75,1,Inf),
                labels=c("0","0-10%","10-25%","25-50%","50-75%", "75-100%","100%"))
